...
class ItemApiTest {
  ...
  @Test
  void unauthenticatedItemListIsRejected() throws Exception {
    mvc.perform(get("/api/v1/items")).andExpect(status().isUnauthorized());
  }

  @Test
  void itemTitleIsValidatedBeforeServiceCall() throws Exception {
    var auth =
  ...
  @Test
  void itemSourceIsValidatedBeforeServiceCall() throws Exception {
    var auth =
        new UsernamePasswordAuthenticationToken(UUID.randomUUID().toString(), null, List.of());
    mvc.perform(
            post("/api/v1/items")
  ...
  @Test
  void progressRangeIsValidatedBeforeServiceCall() throws Exception {
    var auth =
        new UsernamePasswordAuthenticationToken(UUID.randomUUID().toString(), null, List.of());
    mvc.perform(
            post("/api/v1/items/" + UUID.randomUUID() + "/progress")
                .with(authentication(auth))
                .contentType(MediaType.APPLICATION_JSON)
  ...
  @Test
  void malformedItemIdIsRejectedAsBadRequest() throws Exception {
    var auth =
        new UsernamePasswordAuthenticationToken(UUID.randomUUID().toString(), null, List.of());
