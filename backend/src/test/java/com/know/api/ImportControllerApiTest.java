...
class ImportControllerApiTest {
  ...
  @Test
  void unauthenticatedClockifyImportIsRejected() throws Exception {
    mvc.perform(
  ...
  @Test
  void authenticatedClockifyImportPassesEntriesToTheOwnedService() throws Exception {
    ...
    mvc.perform(
            post("/api/v1/imports/clockify")
                ...
                .contentType("application/json")
                .content(
                    "{\"timeentries\":[{\"_id\":\"entry-1\",\"projectName\":\"Java\",\"timeInterval\":{\"start\":\"2026-08-25T10:00:00Z\",\"end\":\"2026-08-25T10:30:00Z\"}}]}"))
  ...
  @Test
  void authenticatedBatchListAndUndoUseTheOwnedService() throws Exception {
    UUID user = UUID.randomUUID(), batchId = UUID.randomUUID();
    when(service.listBatches(user))
        .thenReturn(
            List.of(
                new ClockifyImportService.ImportBatchView(
                    batchId,
                    com.know.domain.TimeSource.IMPORT,
                    2,
                    1,
                    1,
                    java.time.Instant.parse("2026-08-26T10:00:00Z"),
                    null)));
    when(service.undoBatch(user, batchId))
        .thenReturn(new ClockifyImportService.UndoSummary(batchId, 2, 2));
    var auth = new UsernamePasswordAuthenticationToken(user.toString(), null, List.of());
    ...
    mvc.perform(delete("/api/v1/imports/clockify/batches/{id}", batchId).with(authentication(auth)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.deletedEntries").value(2));
