...
class ReportApiTest {
    ...
    @Test void invalidReportPeriodIsRejected() throws Exception {
    ...
    @Test void reportPeriodAndAnchorReachTheOwnedService() throws Exception {
        UUID user = UUID.randomUUID();
        when(service.report(eq(user), eq(ReportService.Period.MONTH), eq(java.time.LocalDate.of(2026, 7, 20))))
                .thenReturn(new ReportService.Report("MONTH", java.time.LocalDate.of(2026, 7, 1), java.time.LocalDate.of(2026, 7, 31), 0, List.of(), List.of(), List.of()));
        var auth = new UsernamePasswordAuthenticationToken(user.toString(), null, List.of());

        mvc.perform(get("/api/v1/reports").param("period", "month").param("anchor", "2026-07-20").with(authentication(auth)))
