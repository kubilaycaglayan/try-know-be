...
class NoteApiTest {
    ...
    @Test void unauthenticatedNoteCreateIsRejected() throws Exception {
    ...
    @Test void blankNoteFieldsAreRejectedBeforeServiceCall() throws Exception {
        var auth = new UsernamePasswordAuthenticationToken(UUID.randomUUID().toString(), null, List.of());
        mvc.perform(post("/api/v1/notes").with(authentication(auth))
                        .contentType("application/json")
