...
class AuthControllerApiTest {
    ...
    @Test void invalidGoogleTokenIsRejectedBeforeAccountLookup() throws Exception {
    ...
    @Test void verifiedGoogleIdentityLinksAnExistingEmail() throws Exception {
        User existing=new User("person@example.com","hash","person");
        when(google.verify("good-token")).thenReturn(Optional.of(new GoogleIdentityVerifier.Identity("google-sub", "person@example.com", "Person")));
        ...
        mvc.perform(post("/api/v1/auth/google").contentType(MediaType.APPLICATION_JSON)
                ...
                .andExpect(status().isOk()).andExpect(jsonPath("$.email").value("person@example.com"));
        org.junit.jupiter.api.Assertions.assertEquals("google-sub", existing.getGoogleSubject());
        verify(users).save(existing);
        verifyNoInteractions(encoder);
    }

    @Test void verifiedGoogleIdentityCreatesAnAccountWithRandomUnusablePassword() throws Exception {
        User created=new User("new@example.com","google-password-hash","New Person");
        when(google.verify("new-token")).thenReturn(Optional.of(new GoogleIdentityVerifier.Identity("new-sub", "new@example.com", "New Person")));
        ...
        org.junit.jupiter.api.Assertions.assertEquals("New Person", account.getValue().getDisplayName());
    ...
    @Test void configuredOriginReceivesCorsPermission() {
        ...
        org.junit.jupiter.api.Assertions.assertEquals("http://localhost", configuration.checkOrigin("http://localhost"));
        org.junit.jupiter.api.Assertions.assertTrue(configuration.getAllowedMethods().contains("GET"));
    }

    @Test void unconfiguredOriginDoesNotReceiveCorsPermission() {
        MockHttpServletRequest request = new MockHttpServletRequest();
        CorsConfiguration configuration = corsConfigurationSource.getCorsConfiguration(request);
