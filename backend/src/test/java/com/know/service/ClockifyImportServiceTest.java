...
class ClockifyImportServiceTest {
    @Test void importCreatesMissingPathAndUsesClockifyIdForDuplicateProtection() {
        PathRepository paths=mock(PathRepository.class); TimeEntryRepository entries=mock(TimeEntryRepository.class); ActivityRepository activities=mock(ActivityRepository.class); ImportBatchRepository batches=mock(ImportBatchRepository.class);
        ...
        verify(entries).save(argThat(saved -> saved.getSource()==TimeSource.IMPORT && saved.getExternalId().equals("clockify-1") && saved.getDurationSeconds()==1800 && result.batchId().equals(saved.getImportBatchId())));
    ...
    @Test void undoBatchDeletesOnlyOwnedImportedEntriesAndActivitiesOnce() {
        PathRepository paths=mock(PathRepository.class); TimeEntryRepository entries=mock(TimeEntryRepository.class); ActivityRepository activities=mock(ActivityRepository.class); ImportBatchRepository batches=mock(ImportBatchRepository.class);
        UUID user=UUID.randomUUID(); ImportBatch batch=new ImportBatch(user,TimeSource.IMPORT);
