...
class ReportServiceTest {
    @Test void monthlyReportShowsDailyPathAndItemBreakdownsWithClippedIntervals() {
        ...
        TimeEntry crossing = new TimeEntry(user, path.getId(), item.getId(), monthStart.minusSeconds(30), "crossing", TimeSource.IMPORT);
        crossing.stop(monthStart.plusSeconds(30));
        TimeEntry later = new TimeEntry(user, path.getId(), item.getId(), Instant.parse("2026-07-12T10:00:00Z"), "later", TimeSource.IMPORT);
        later.stop(Instant.parse("2026-07-12T10:10:00Z"));
        when(entries.findOverlappingByUserId(eq(user), any(), any())).thenReturn(List.of(later, crossing));
        ...
        ReportService.Report report = new ReportService(entries, paths, items).report(user, ReportService.Period.MONTH, LocalDate.of(2026, 7, 20));
    ...
    @Test void yearReportContainsEveryUtcDay() {
        TimeEntryRepository entries = mock(TimeEntryRepository.class);
        when(entries.findOverlappingByUserId(any(), any(), any())).thenReturn(List.of());
        ReportService.Report report = new ReportService(entries, mock(PathRepository.class), mock(ItemRepository.class))
