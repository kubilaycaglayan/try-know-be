 ...
 *
 * Each test registers a fresh user so tests are independent and can run in any order.
 * Covers every criterion that was moved to .agents/criterias/completed/.
 */
...
class KnowIntegrationTest {
    ...
    String registerAndLogin(String email, String password) {
        ...
        assertEquals(HttpStatus.OK, res.getStatusCode(),
            "Login/register failed: " + res.getBody());
    ...
    String json(String... kvPairs) {
        ...
        for (int i = 0; i < kvPairs.length; i += 2) {
            ...
            sb.append('"').append(kvPairs[i]).append('"').append(':')
              .append('"').append(kvPairs[i + 1]).append('"');
        }
    ...
    void registrationCreatesUserAndLoginReturnsJwt() {
        ...
        ResponseEntity<JsonNode> reg = post("/api/v1/auth/register", null, json("email", email, "password", pw));
        ...
        ResponseEntity<JsonNode> dup = post("/api/v1/auth/register", null, json("email", email, "password", pw));
        assertEquals(HttpStatus.CONFLICT, dup.getStatusCode());

        // Login succeeds
        ResponseEntity<JsonNode> login = post("/api/v1/auth/login", null, json("email", email, "password", pw));
        assertEquals(HttpStatus.OK, login.getStatusCode());
        assertNotNull(login.getBody().get("token").asText());

        // Wrong password is rejected
        ResponseEntity<JsonNode> bad = post("/api/v1/auth/login", null, json("email", email, "password", "WrongPassword!"));
    ...
    void emailUniquenessIsCaseInsensitive() {
        ...
        ResponseEntity<JsonNode> dup = post("/api/v1/auth/register", null, json("email", upper, "password", "SecurePassword123!"));
        assertEquals(HttpStatus.CONFLICT, dup.getStatusCode());
    }
    ...
    void pathLifecycleCreateReadUpdateArchive() {
        ...
        // Create
        ResponseEntity<JsonNode> created = post("/api/v1/paths", token,
        ...
        ResponseEntity<JsonNode> updated = put("/api/v1/paths/" + pathId, token,
            "{\"name\":\"Algorithms Updated\",\"description\":\"Updated desc\",\"color\":\"#EF4444\"}");
        ...
        for (JsonNode n : list.getBody()) {
            if (n.get("id").asText().equals(pathId)) { found = true; break; }
        }
        ...
        assertEquals("ARCHIVED", get("/api/v1/paths/" + pathId, token).getBody().get("status").asText());
    }

    @Test
    void pathColorValidationRejectsInvalidHex() {
        String token = freshToken();
        ResponseEntity<JsonNode> bad = post("/api/v1/paths", token,
    ...
    void pathsHaveColorsDefaultAndCustom() {
        ...
        ResponseEntity<JsonNode> defaultPath = post("/api/v1/paths", token,
            "{\"name\":\"No Color Path\",\"description\":null}");
        assertEquals(HttpStatus.CREATED, defaultPath.getStatusCode());
        assertFalse(defaultPath.getBody().get("color").asText().isBlank(),
            "default color should be set");

        // Custom color is stored
        ResponseEntity<JsonNode> colored = post("/api/v1/paths", token,
            "{\"name\":\"Blue Path\",\"color\":\"#2563EB\"}");
    ...
    void itemLifecycleCreateUpdateProgressComplete() {
        ...
        boolean found = false;
        for (JsonNode n : list.getBody()) {
            if (n.get("id").asText().equals(itemId)) { found = true; break; }
        }
        assertTrue(found);

        // Update progress
        ResponseEntity<JsonNode> progressed = post("/api/v1/items/" + itemId + "/progress", token,
            "{\"progress\":50}");
        ...
        ResponseEntity<JsonNode> completed = put("/api/v1/items/" + itemId, token,
            "{\"title\":\"Chapter 1\",\"type\":\"COURSE\",\"status\":\"COMPLETED\","
            + "\"pathIds\":[\"" + pathId + "\"],\"tags\":[\"study\"]}");
    ...
    void itemSourceFieldPersistedAndEditable() {
        ...
        ResponseEntity<JsonNode> updated = put("/api/v1/items/" + itemId, token,
    ...
    void allSupportedItemTypesCanBeCreated() {
        ...
        for (String type : new String[]{
            ...
            assertEquals(HttpStatus.CREATED, item.getStatusCode(),
                "Should create item of type " + type);
    ...
    void archivedPathCannotReceiveNewItemMembership() {
        String token = freshToken();
        ResponseEntity<JsonNode> path = post("/api/v1/paths", token,
            "{\"name\":\"Archive Me\",\"description\":null}");
        String pathId = path.getBody().get("id").asText();
    ...
    void notesCanBeCreatedAndEdited() {
        String token = freshToken();
        ResponseEntity<JsonNode> path = post("/api/v1/paths", token,
            "{\"name\":\"Notes Path\",\"description\":null}");
        String pathId = path.getBody().get("id").asText();

        // Create note attached to path
        ResponseEntity<JsonNode> note = post("/api/v1/notes", token,
            "{\"pathId\":\"" + pathId + "\",\"title\":\"My Note\",\"content\":\"Note content here\"}");
        ...
        boolean found = false;
        for (JsonNode n : list.getBody()) {
            if (n.get("id").asText().equals(noteId)) { found = true; break; }
        }
        assertTrue(found);
    ...
    void noteForeignUserCannotEdit() {
        ...
        ResponseEntity<JsonNode> note = post("/api/v1/notes", ownerToken,
            "{\"title\":\"Private\",\"content\":\"secret\"}");
        assertEquals(HttpStatus.OK, note.getStatusCode());
        String noteId = note.getBody().get("id").asText();

        // Foreign user edit is rejected
        ResponseEntity<JsonNode> reject = put("/api/v1/notes/" + noteId, foreignToken,
            "{\"title\":\"Leaked\",\"content\":\"nope\"}");
    ...
    void timerStartStopCycleAndOneTimerInvariant() {
        ...
        // Start a timer
        ResponseEntity<JsonNode> started = post("/api/v1/timers", token,
            "{\"description\":\"Study session\",\"source\":\"WEB\"}");
        ...
        ResponseEntity<JsonNode> dup = post("/api/v1/timers", token,
            "{\"description\":\"Another session\",\"source\":\"WEB\"}");
        assertEquals(HttpStatus.CONFLICT, dup.getStatusCode());
    ...
    void timerCanBeCancelled() {
        String token = freshToken();
        ResponseEntity<JsonNode> started = post("/api/v1/timers", token,
            "{\"description\":\"To be cancelled\",\"source\":\"WEB\"}");
        assertEquals(HttpStatus.CREATED, started.getStatusCode());
        String timerId = started.getBody().get("id").asText();
    ...
    void timerPreservesIosSource() {
        String token = freshToken();
        ResponseEntity<JsonNode> started = post("/api/v1/timers", token,
            "{\"description\":\"iOS session\",\"source\":\"IOS\"}");
        assertEquals(HttpStatus.CREATED, started.getStatusCode());
    ...
    void timerConfigureRunningUpdatesPathAndStartTime() {
        String token = freshToken();
        ResponseEntity<JsonNode> path = post("/api/v1/paths", token,
            "{\"name\":\"Config Path\",\"description\":null}");
        String pathId = path.getBody().get("id").asText();

        ResponseEntity<JsonNode> started = post("/api/v1/timers", token,
            "{\"description\":\"Config test\",\"source\":\"WEB\"}");
        assertEquals(HttpStatus.CREATED, started.getStatusCode());
        String timerId = started.getBody().get("id").asText();

        String pastTime = Instant.now().minus(10, ChronoUnit.MINUTES).toString();
        ResponseEntity<JsonNode> configured = put("/api/v1/timers/" + timerId, token,
            "{\"pathId\":\"" + pathId + "\",\"startedAt\":\"" + pastTime + "\",\"description\":\"Updated\"}");
    ...
    void timerRejectsItemNotAttachedToPath() {
        String token = freshToken();
        ResponseEntity<JsonNode> path = post("/api/v1/paths", token,
            "{\"name\":\"Path A\",\"description\":null}");
        String pathId = path.getBody().get("id").asText();

        ResponseEntity<JsonNode> item = post("/api/v1/items", token,
            "{\"title\":\"Unattached Item\"}");
        String itemId = item.getBody().get("id").asText();

        // Item is not in Path A, so it should be rejected
        ResponseEntity<JsonNode> timerStart = post("/api/v1/timers", token,
            "{\"pathId\":\"" + pathId + "\",\"itemId\":\"" + itemId + "\","
    ...
    void manualTimeEntryIsCreated() {
        ...
        ResponseEntity<JsonNode> entry = post("/api/v1/time-entries", token,
    ...
    void activityStreamRecordsItemAndTimerEvents() {
        ...
        ResponseEntity<JsonNode> item = post("/api/v1/items", token,
            "{\"title\":\"Activity Check\",\"type\":\"COURSE\"}");
        String itemId = item.getBody().get("id").asText();

        // Start timer, then expect TIMER_STARTED
        ResponseEntity<JsonNode> timerRes = post("/api/v1/timers", token,
            "{\"description\":\"Activity timer\",\"source\":\"WEB\"}");
    ...
    void noteAttachedToActivityIsOwnershipScoped() {
        ...
        ResponseEntity<JsonNode> note = post("/api/v1/notes", token,
            "{\"activityId\":\"" + activityId + "\",\"title\":\"Activity Note\","
    ...
    void searchReturnsMatchingPathsItemsAndNotes() {
        ...
        post("/api/v1/paths", token,
            "{\"name\":\"" + unique + " Path\",\"description\":null}");
        post("/api/v1/items", token,
            "{\"title\":\"" + unique + " Item\"}");
        post("/api/v1/notes", token,
            "{\"title\":\"" + unique + " Note\",\"content\":\"some content\"}");
    ...
    void pathSummaryIncludesTrackedTimeAndItems() {
        String token = freshToken();
        ResponseEntity<JsonNode> path = post("/api/v1/paths", token,
            "{\"name\":\"Summary Path\",\"description\":null}");
        String pathId = path.getBody().get("id").asText();
        ...
        post("/api/v1/time-entries", token,
            "{\"pathId\":\"" + pathId + "\",\"itemId\":\"" + itemId + "\","
            + "\"startedAt\":\"" + start + "\",\"endedAt\":\"" + end + "\","
        ...
        assertTrue(summary.getBody().get("trackedSeconds").asLong() > 0,
            "tracked seconds should be positive");
        assertTrue(summary.getBody().get("itemIds").size() > 0,
            "summary should include item ids");
    }
    ...
    void pathsAreOrderedByMostRecentUse() {
        ...
        ResponseEntity<JsonNode> pathA = post("/api/v1/paths", token,
            "{\"name\":\"Older Path\",\"description\":null}");
        ResponseEntity<JsonNode> pathB = post("/api/v1/paths", token,
            "{\"name\":\"Newer Path\",\"description\":null}");
        String pathIdB = pathB.getBody().get("id").asText();

        // Touch path B by updating it so its updatedAt is more recent
        put("/api/v1/paths/" + pathIdB, token,
            "{\"name\":\"Newer Path\",\"description\":\"touched\"}");

        ResponseEntity<JsonNode> list = get("/api/v1/paths", token);
        assertEquals(HttpStatus.OK, list.getStatusCode());
        assertTrue(list.getBody().isArray() && list.getBody().size() >= 2);
        // First path in the list should be pathB (most recently updated)
        assertEquals(pathIdB, list.getBody().get(0).get("id").asText(),
    ...
    void clockifyImportCreatesEntriesAndPaths() {
        ...
        for (JsonNode n : paths.getBody()) {
            if (uniqueProject.equalsIgnoreCase(n.get("name").asText())) { pathFound = true; break; }
        }
    ...
    void clockifyImportBatchesListedAndUndone() {
        ...
        for (JsonNode n : batches.getBody()) {
            if (n.get("id").asText().equals(batchId)) { batchFound = true; break; }
        }
        ...
        assertTrue(undo.getBody().get("deletedEntries").asLong() > 0,
    ...
    void oversizedItemTitleIsRejected() {
        String token = freshToken();
        ResponseEntity<JsonNode> result = post("/api/v1/items", token,
            "{\"title\":\"" + "x".repeat(241) + "\"}");
        assertEquals(HttpStatus.BAD_REQUEST, result.getStatusCode());
    }
    ...
    void malformedUuidInPathParameterReturnsBadRequest() {
        ...
        assertEquals(HttpStatus.BAD_REQUEST,
            get("/api/v1/paths/not-a-uuid", token).getStatusCode());
        assertEquals(HttpStatus.BAD_REQUEST,
            get("/api/v1/items/not-a-uuid/progress", token).getStatusCode());
    }
    ...
    void usersCannotAccessEachOthersResources() {
        ...
        ResponseEntity<JsonNode> alicePath = post("/api/v1/paths", aliceToken,
            "{\"name\":\"Alice's Path\",\"description\":null}");
        ...
        ResponseEntity<JsonNode> bobItem = post("/api/v1/items", bobToken,
    ...
    void oversizedTimerDescriptionIsRejected() {
        String token = freshToken();
        ResponseEntity<JsonNode> result = post("/api/v1/timers", token,
    ...
    void completingItemCreatesCompletionActivity() {
        String token = freshToken();
        ResponseEntity<JsonNode> item = post("/api/v1/items", token,
            "{\"title\":\"Completable\",\"type\":\"COURSE\"}");
        String itemId = item.getBody().get("id").asText();

        put("/api/v1/items/" + itemId, token,
    ...
    void reducingProgressFromCompletedReopensItem() {
        ...
        for (JsonNode n : updated.getBody()) {
            if (n.get("id").asText().equals(itemId)) {
                ...
                assertNotEquals("COMPLETED", n.get("status").asText(),
    ...
    void authRateLimiterBlocksExcessiveAttempts() {
        ...
        for (int i = 0; i < 12; i++) {
            last = post("/api/v1/auth/login", null,
    ...
    void pathNameCannotBeBlankOrExceedMaxLength() {
        String token = freshToken();
        assertEquals(HttpStatus.BAD_REQUEST,
            post("/api/v1/paths", token, "{\"name\":\"\"}").getStatusCode());
        assertEquals(HttpStatus.BAD_REQUEST,
    ...
    void pathDescriptionIsOptionalAndPersistedWhenProvided() {
        ...
        // Without description
        ResponseEntity<JsonNode> noDesc = post("/api/v1/paths", token,
            "{\"name\":\"No Desc Path\"}");
        assertEquals(HttpStatus.CREATED, noDesc.getStatusCode());
        assertTrue(noDesc.getBody().get("description").isNull() || noDesc.getBody().get("description").asText().isEmpty());

        // With description
        ResponseEntity<JsonNode> withDesc = post("/api/v1/paths", token,
