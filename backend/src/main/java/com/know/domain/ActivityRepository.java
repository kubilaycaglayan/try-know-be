...
public interface ActivityRepository extends JpaRepository<Activity,UUID>{
 ...
 @Query("select a from Activity a where a.userId=:userId and (:from is null or a.occurredAt>=:from) and (:to is null or a.occurredAt<=:to) and (:pathId is null or a.pathId=:pathId) and (:itemId is null or a.itemId=:itemId) and (:type is null or a.type=:type) order by a.occurredAt desc")
