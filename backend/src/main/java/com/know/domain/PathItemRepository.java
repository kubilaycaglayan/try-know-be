...
public interface PathItemRepository extends JpaRepository<PathItem,PathItemId>{
 interface ItemPathProjection { UUID getItemId(); UUID getPathId(); }
 @Query("select p.id.itemId from PathItem p where p.id.pathId=:pathId") List<UUID> findItemIds(UUID pathId);
