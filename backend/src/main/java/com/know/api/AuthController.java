...
public class AuthController {
    private final UserRepository users; private final PasswordEncoder encoder; private final com.know.security.AuthAttemptLimiter limiter; private final com.know.security.GoogleIdentityVerifier google; private final SecretKey key;
    private static final SecureRandom RANDOM = new SecureRandom();
    public AuthController(UserRepository users, PasswordEncoder encoder, com.know.security.AuthAttemptLimiter limiter, com.know.security.GoogleIdentityVerifier google, @Value("${app.jwt-secret}") String secret) { this.users=users; this.encoder=encoder; this.limiter=limiter; this.google=google; this.key=Keys.hmacShaKeyFor(secret.getBytes(StandardCharsets.UTF_8)); }
    ...
    @PostMapping("/google") public AuthResponse google(@Valid @RequestBody GoogleRequest request, jakarta.servlet.http.HttpServletRequest httpRequest) {
        checkRate(httpRequest, "google");
        var identity=google.verify(request.idToken()).orElseThrow(()->new ResponseStatusException(HttpStatus.UNAUTHORIZED,"Invalid Google identity token"));
        User user=users.findByGoogleSubject(identity.subject()).orElseGet(() -> users.findByEmailIgnoreCase(identity.email()).orElse(null));
        if (user == null) {
            byte[] random = new byte[32]; RANDOM.nextBytes(random);
            user=new User(identity.email(), encoder.encode(Base64.getUrlEncoder().withoutPadding().encodeToString(random)), identity.displayName());
        } else if (user.getGoogleSubject() != null && !user.getGoogleSubject().equals(identity.subject())) {
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED,"Google account is already linked");
        }
