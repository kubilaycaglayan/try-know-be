...
public class KnowledgeService {
    ...
    public ItemView updateItem(
        ...
        if (previousProgress != saved.getProgress()) {
            progressRepository.save(
                new ProgressEntry(userId, id, previousProgress, saved.getProgress())
            );
            activityRepository.save(
                new Activity(
                    userId,
    ...
    private void attach(
        ...
        if (pathIds != null) {
            for (int n = 0; n < pathIds.size(); n++) {
                ...
                Path path =
                    paths.findByIdAndUserId(pathId, userId)
                        .orElseThrow(
                            () ->
                                new ResponseStatusException(
                                    HttpStatus.BAD_REQUEST,
                                    "Path does not belong to user"
                                )
                        );
                if (path.getStatus() != PathStatus.ACTIVE && !existingPathIds.contains(pathId)) {
                    throw new ResponseStatusException(
                        HttpStatus.BAD_REQUEST,
                        "Archived paths cannot receive new items"
                    );
                }
    ...
    public NoteView createNote(
        ...
        int targets = (pathId != null ? 1 : 0) + (itemId != null ? 1 : 0) + (activityId != null
            ? 1
            : 0);
        if (targets > 1) {
            throw new ResponseStatusException(
                HttpStatus.BAD_REQUEST,
                "A note can have only one target"
            );
        }
        if (pathId != null) {
            paths.findByIdAndUserId(pathId, userId)
                .orElseThrow(
                    () ->
                        new ResponseStatusException(HttpStatus.NOT_FOUND, "Path not found")
                );
        }
        if (itemId != null) {
    ...
    public List<NoteView> listNotes(UUID userId) {
        return notes.findAllByUserIdOrderByUpdatedAtDesc(userId, PageRequest.of(0, 100))
            .stream()
            .map(this::noteView)
            .toList();
    }
    ...
    public NoteView updateNote(UUID userId, UUID id, String title, String content) {
        Note note =
            notes.findByIdAndUserId(id, userId)
                .orElseThrow(
                    () ->
