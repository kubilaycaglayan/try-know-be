...
describe('TimelineView', () => {
  ...
  it('saves a note attached to an activity', async () => {
    ...
    const addNoteButton = wrapper.findAll('button.text-button').find(button => button.text() === 'Add note')
    expect(addNoteButton).toBeDefined()
    await addNoteButton!.trigger('click')
    await wrapper.get('input[aria-label="Activity note title"]').setValue('Key idea')
    await wrapper.get('textarea[aria-label="Activity note content"]').setValue('Spaced repetition helps.')
    const saveNoteButton = wrapper.findAll('button.primary').find(button => button.text() === 'Save note')
    ...
    expect(vi.mocked(api)).toHaveBeenCalledWith('/notes', expect.objectContaining({
      method: 'POST',
      body: JSON.stringify({ activityId: 'activity-1', title: 'Key idea', content: 'Spaced repetition helps.' })
    }))
  })
})
