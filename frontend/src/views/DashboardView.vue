...
const timerItems = computed(() => pathId.value ? items.value.filter(item => item.pathIds.includes(pathId.value)) : items.value)
watch(pathId, value => { if (value && !timerItems.value.some(item => item.id === itemId.value)) itemId.value = '' })
